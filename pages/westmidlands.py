...
# =========================
# Load base charger data (from provided Sheet)
# =========================
SHEET_URL = "https://docs.google.com/spreadsheets/d/1xjD-NH6rX7_ueOU89jxZsKXahURfRAyA/edit?usp=sharing&ouid=118120094376416558501&rtpof=true&sd=true%22"
df = load_csv_from_gsheet(SHEET_URL)
 
 
def _col(df0, *candidates):
    for c in candidates:
        if c in df0.columns:
            return c
...
west_midlands_gdf = gpd.GeoDataFrame(df, geometry="geometry", crs="EPSG:4326").copy()
                ...
                if outers:
                    try:
                        ...
                        if inners:
                            mp = Polygon(
                                mp.exterior.coords,
                                holes=[i.exterior.coords for i in inners if i.is_valid],
                            )
                        if not mp.is_empty:
                            geoms.append(mp)
                    except Exception:
